...
for (const room of rooms) {
  for (const [surfaceIndex, surface] of room.entries) {
    const { width, height, paint, openings } = surface;
    const surfaceArea = Math.round(width * height) / 100;
    // TODO get index
    console.log(`\nSurface area of ${surfacesString} #${surfaceIndex + 1} is ${surfaceArea} ${areaUnit}`);
...
async function getPaintForSurface(thingToBePainted) {
  ...
  if (paints.length > 0) {
    console.log("\nPaints used so far:");
    paints.forEach((p, i) => console.log(`  ${i + 1}. "${p.paintName}"`));
    ...
    if (index >= 0 && index < paints.length) {
      ...
      return { ...paints[index], paintCoats };
    }
  }
  ...
  // TODO side effect!
  paints.push({ paintName, paintCoats, paintCoverage });
